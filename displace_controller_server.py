...
def send_string(text: str):
    ...
    for char in text:
        try:
            ...
            # 1. Nhấn SHIFT nếu cần
            if needs_shift:
                # Gửi sự kiện nhấn phím SHIFT (giữ)
                ui.write(ecodes.EV_KEY, ecodes.KEY_LEFTSHIFT, 1)
                ui.syn()

            # 2. Gõ phím chính (nhấn xuống rồi nhả ra)
            ui.write(ecodes.EV_KEY, key_code, 1) # Nhấn xuống
            ui.write(ecodes.EV_KEY, key_code, 0) # Nhả ra
            ui.syn() # Đồng bộ hóa để gửi sự kiện

            # 3. Nhả SHIFT nếu đã được nhấn
            if needs_shift:
                # Gửi sự kiện nhả phím SHIFT
...
class App:
    ...
    def rx_write(cls, value, options):
        ...
        typ, dx, dy, btn, down, dwheel, hwheel, k1, k2 = struct.unpack(PKT_FMT, data)
        ...
        if typ == 1: # "move"
            ...
            if cls.tx_char:
                ...
                cls.tx_char.set_value(b"abcdefghik")
